...
class AES:
    ...
    def encrypt(self, data: bytes, iv=None) -> tuple:
        ...
        cipher = self._encrypt_CBC(
            blocks, iv) if self.mode == CBC else self._encrypt_ECB(blocks)
    ...
    def decrypt(self, data: bytes, iv=None) -> bytes:
        ...
        decrypted = self._decrypt_CBC(
            blocks, iv) if self.mode == CBC else self._decrypt_ECB(blocks)
